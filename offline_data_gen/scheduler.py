...
class Scheduler:
    def __init__(self, nodes, init_time, policy, config, mode):
    ...
    def submit(self, jobs):
        ...
        for job in jobs:
            if job.priority_from_log:
                job.priority = priority_reset(job)
            else:
                job.priority = self._config["PriorityWeightJobSize"] * (job.nodes / self._cluster.num_proc)
