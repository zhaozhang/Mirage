...
class Simulator:
    ...
    def run_group_dependency(self, groups, infer_func, train_freq=300, infer_freq=1, sample_time_step=144,
                             ...
                             max_store_reward=1, infer_lower_bound=-5.0, infer_upper_bound=5.0, offline=False,
                             offline_sample_index=0):
        """
        ...
        while True:
            ...
            for i in range(0, len(groups)):
                ...
                if infer_counter == infer_freq:
                    if offline:
                        ...
                        if (pred_job.job_id, next_job.job_id) not in self._active_history.keys():
                            continue
                        target_data = self._active_history[pred_job.job_id, next_job.job_id][0][0]
                        if len(target_data) < sample_time_step:
                            continue
                        expected_reward = infer_func([target_data])
                        logging.info(f"Simulator Time: {self._time}, Complete inference.")
            ...
            for item in wait_reward_pool:
                if item[0].job_id in reward_collection.keys():
                    ...
                    if not offline:
                        expected_reward = infer_func([self._active_reward[dual_job_id_pair][0][0]])
                        logging.info(
                            f"Simulator time: {self._time}, Reward Update: Expected reward: {expected_reward}, "
                            f"Actual reward: {reward_collection[item[0].job_id][1]}, "
                            f"Error: {expected_reward - reward_collection[item[0].job_id][1]}, ")
                    else:
                        logging.info(
            ...
            if train_func is not None and (reward_counter == max_store_reward):
                ...
                train_counter = 0
                reward_counter = 0
            if train_func is not None and curr_reward == total_reward:
    ...
    def reward(self):
        return self._scheduler.reward_pool
