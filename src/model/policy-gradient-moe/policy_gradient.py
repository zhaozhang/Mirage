...
class PolicyGradient:
    ...
    def solve_environment(self, init_epoch=0, save_checkpoint=0):
        """
...
def main():
    ...
    parser.add_argument("-base_dir", required=True, help="Experts directory")
    ...
    parser.add_argument("-base_prefix", required=True, help="The prefix of expert file name")
    ...
    if os.path.abspath(args.base_dir) != os.path.abspath(args.output_dir):
        ...
        for i in range(0, args.num_experts):
            shutil.copy(os.path.join(args.base_dir, "{}_{}.pt".format(args.base_prefix, i)),
                        os.path.join(args.output_dir, "{}_{}.pt".format(args.base_prefix, i)))

    policy_gradient = PolicyGradient(train_cfg=train_cfg, use_cuda=args.use_cuda, num_expert=args.num_experts,
                                     experts_dir=args.output_dir, experts_name=args.base_prefix, sim_config=config,
